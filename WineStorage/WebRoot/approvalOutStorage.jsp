...
<section id="container" class="">
  ...
  <section id="main-content">
    <section class="wrapper">
      <div class="row">
        <div class="col-lg-12">
          <h3 class="page-header"><i class="fa fa-user-md"></i> 出库审批</h3>
          <ol class="breadcrumb">
            <li><i class="fa fa-home"></i><a href="index.jsp">主页</a></li>
            <li><i class="icon_documents_alt"></i>审批</li>
            <li><i class="fa fa-user-md"></i>出库审批</li>
          </ol>
        </div>
      </div>
      <div class="row">
              ...
              <div class="tab-content">
                <div id="pending" class="tab-pane active">
                  <table class="table bootstrap-datatable">
                    ...
                    <tbody id="pendingOutStorageList">
                ...
                <div id="rejected" class="tab-pane">
                  <table class="table bootstrap-datatable">
                    ...
                    <tbody id="rejectedOutStorageList">
...
<form name="failOutStorageForm" method="post" action="<%=request.getContextPath()%>/failOutStorage.action">
  <input type="hidden" id="failId" name="outStorageId" value=""/>
</form>

<form name="passOutStorageForm" method="post" action="<%=request.getContextPath()%>/passOutStorage.action">
  <input type="hidden" id="passId" name="outStorageId" value=""/>
</form>
...
<script type="text/javascript">
  ...
  function failOutStorage(id){
    document.getElementById("failId").value = id;
    document.failOutStorageForm.submit();
  }
  function passOutStorage(id){
    document.getElementById("passId").value = id;
    document.passOutStorageForm.submit();
  }
  ...
  function replaceModelSelectHtml(){
    ...
    for(var i = 0; i < outStorages.length; i++) {
      ...
      if (outStorage["status"] == "0") {
        var content = '<strong>条形码: </strong>' + outStorage["itemBarcode"] + '<br/>' +
                '<strong>品种: </strong>' + outStorage["itemVariety"] + '<br/>'+
                '<strong>规格: </strong>' + outStorage["itemStandard"] + '<br/>'+
                '<strong>库存: </strong>' + outStorage["itemStorage"] + '<br/>';
        Html += '<tr>' +
                '<td>' + (index++) + '</td>' +
                ...
                '<td>' + outStorage["place"] + '</td>' +
                '<td>' + outStorage["agent"] + '</td>' +
                '<td>待审</td>' +
                '<td><div class="btn-group">' +
                '<a class="btn btn-warning pop" data-toggle="popover" data-placement="left" data-content="不通过" onclick="failOutStorage(\'' + outStorage["id"] + '\')"><i class="icon_pencil-edit_alt"></i></a>' +
                '<a class="btn btn-success pop" data-toggle="popover" data-placement="left" data-content="通过" onclick="passOutStorage(\'' + outStorage["id"] + '\')"><i class="icon_pencil-edit_alt"></i></a></div></tr>';
      }

    }
    document.getElementById("pendingOutStorageList").innerHTML=Html;

    Html = '';
    index = 1;
    for(var i = 0; i < outStorages.length; i++) {
      var outStorage = outStorages[i];
      if (outStorage["status"] == "1") {
        var content = '<strong>条形码: </strong>' + outStorage["itemBarcode"] + '<br/>' +
                '<strong>品种: </strong>' + outStorage["itemVariety"] + '<br/>'+
                '<strong>规格: </strong>' + outStorage["itemStandard"] + '<br/>'+
                '<strong>库存: </strong>' + outStorage["itemStorage"] + '<br/>';
    ...
    document.getElementById("rejectedOutStorageList").innerHTML=Html;

    Html = '';
    index = 1;
    for(var i = 0; i < outStorages.length; i++) {
      var outStorage = outStorages[i];
      if (outStorage["status"] == "2") {
        var content = '<strong>条形码: </strong>' + outStorage["itemBarcode"] + '<br/>' +
                '<strong>品种: </strong>' + outStorage["itemVariety"] + '<br/>'+
                '<strong>规格: </strong>' + outStorage["itemStandard"] + '<br/>'+
                '<strong>库存: </strong>' + outStorage["itemStorage"] + '<br/>';
      ...
      document.getElementById("pendedOutStorageList").innerHTML=Html;
