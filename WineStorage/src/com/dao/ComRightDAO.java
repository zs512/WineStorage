...
/**
 	* A data access object (DAO) providing persistence and search support for ComRight entities.
 			* Transaction control of the save(), update() and delete() operations 
		can directly support Spring container-managed transactions or they can be augmented	to handle user-managed Spring transactions. 
		Each of these methods provides additional information for how to configure it for the desired type of transaction control. 	
	 * @see com.domain.ComRight
  * @author MyEclipse Persistence Tools 
 */
    @Transactional   
public class ComRightDAO  {
	     private static final Logger log = LoggerFactory.getLogger(ComRightDAO.class);
		//property constants
    ...
    public ComRight findById( java.lang.String id) {
        log.debug("getting ComRight instance with id: " + id);
        try {
            ComRight instance = (ComRight) getCurrentSession()
                    .get("com.domain.ComRight", id);
            return instance;
        } catch (RuntimeException re) {
    ...
    public List<ComRight> findByExample(ComRight instance) {
        ...
        try {
            List<ComRight> results = (List<ComRight>) getCurrentSession() .createCriteria("com.domain.ComRight").add( create(instance) ).list();
    ...
    public ComRight merge(ComRight detachedInstance) {
        ...
        try {
            ComRight result = (ComRight) getCurrentSession()
